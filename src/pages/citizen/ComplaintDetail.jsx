...
import { formatDate, formatDateTime, timeAgo, toPercent } from "../../utils/formatters";
      ...
      <div className="row g-4">
        ...
        <div className="col-12 col-xl-4">
          <div className="stack-4">
            ...
            <Card padding="lg">
              <CardHeader
                title="Possible Related Complaints"
                subtitle="AI detected similar reports in the same area"
              />
              {complaint.duplicateAnalysis?.isPossibleDuplicate ? (
                <>
                  <p className="related__intro mb-2">
                    ⚠️ Possible Duplicate Reports
                  </p>
                  <p className="text-muted-soft small mb-3">
                    {complaint.related.length} similar complaints found
                  </p>
                  <p className="mb-3">
                    <strong>Similarity:</strong> {toPercent(complaint.duplicateAnalysis.similarity)}
                    <br />
                    <strong>Common Problem:</strong> {complaint.title}
                  </p>
                  <Button variant="secondary" block icon="bi-files" to={PATHS.CITIZEN_COMPLAINTS}>
                    View Related Complaints
                  </Button>
                </>
              ) : (
                <RelatedComplaints items={complaint.related} title="Possible Related Complaints" />
              )}
            </Card>
          </div>
        </div>
