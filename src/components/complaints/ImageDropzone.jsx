 ...
 */
export default function ImageDropzone({ onAnalysis, onFileChange, disabled, selectedCategory, complaintText = "" }) {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          const formattedResult = {
            ...
            detected_category: result.detected_category || result.category,
            label: result.label || result.categoryLabel || result.category,
            confidence: Number(result.confidence ?? 0),
            severity: result.severity || "Medium",
            department: result.department || "Municipal Department",
            reason: result.reason || "Image attached successfully.",
            description: result.description || "",
            tags: result.tags || [],
          };

          if (!formattedResult.valid || !formattedResult.relevant) {
            setError(formattedResult.reason || "This image does not appear relevant to the selected complaint.");
            setStatus("done");
          ...
          setVision(unavailable);
          setStatus("done");
          setError("");
          onAnalysis?.(unavailable);
        }
      }, 1000),
    );
  };
  ...
  return (
    <div className="dropzone-wrap">
          ...
          <div className="upload__body">
            ...
            {status === "done" && (
              <p className="upload__status upload__status--ok">
                ...
                {vision?.analysisAvailable === false
      ...
      {vision && vision.analysisAvailable === false && vision.reason && (
        <></>
      )}
        ...
        <div className="vision">
          ...
          <div className="vision__grid">
            <div className="vision__cell">
              <span className="vision__label">
                {vision.analysisAvailable === false ? "Suggested issue" : "Detected"}
              </span>
